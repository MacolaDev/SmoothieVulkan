...
int SmoothieEditor::Tool_Camera::create()
{
    ...
    const auto* __drawing_class = dynamic_cast<Smoothie::DeferredRendering::Drawing*>(SmoothieCore::getDrawingClass());
...
void SmoothieEditor::Tool_Camera::destroy()
{
    m_pCore.reset();
    m_Icon.destroy();
}

void SmoothieEditor::Tool_Camera::on_ui_window_draw()
{

    //***************************** Draw window ****************************//
    if (m_toolFlags & static_cast<int>(BaseTool_Flags::isWindowVisible)) return;
    ...
    if (ImGui::DragFloat("Camera FOV", &m_fov, 0.1f, 1.0f, 120.0f))
    {
        ...
        m_toolFlags |= static_cast<int>(BaseTool_Flags::isToolUpdated);
    }
    ImGui::End();
}

void SmoothieEditor::Tool_Camera::on_ui_icon_draw()
{
	...
	if (m_toolFlags & static_cast<int>(BaseTool_Flags::isToolUpdated))
	{
...
void SmoothieEditor::Tool_Camera::on_command_execution_finish()
{
	SmoothieCore::updateCameraData(m_camera);
}

void SmoothieEditor::Tool_Camera::on_scene_load(const Smoothie::XML::Element &rootElement)
{
...
static std::string _Vector3_to_string(const SmoothieMath::Vector3& vec)
{
	return std::to_string(vec.x) + " " + std::to_string(vec.y) + " " + std::to_string(vec.z);
}

void SmoothieEditor::Tool_Camera::on_scene_save(Smoothie::XML::Element &rootElement)
{
