...
//This is only here because I want to completly disable tab bar in my main window. 
...
int SmoothieEditor::Swizzler::create()
{
	...
	__push_constant.size = sizeof(SmoothieMath::Matrix4x4);
...
void SmoothieEditor::Swizzler::draw(VkCommandBuffer commandBuffer) const
{
	...
	vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(SmoothieMath::Matrix4x4), &matrix);
...
void SmoothieEditor::RenderingManager::draw(VkCommandBuffer commandBuffer, unsigned int currentFrame) const
{
	...
	//************************************************ EDITOR PASS ***********************************************//
	auto& __editor_window = SmoothieEditor::getCore().editor_window;
	auto& __camera_window = __editor_window.camera_window;

	beginInfo.renderPass = resultRenderPass;
	beginInfo.framebuffer = resultFramebuffer;
	vkCmdBeginRenderPass(commandBuffer, &beginInfo, VK_SUBPASS_CONTENTS_INLINE);

	//Draw finished image to the result framebuffer
	...
	if (__camera_window.__selected_pass == 0) //Render only objects if swizzling is not needed
	{
		//Editor 3d objects go here
	...
	if (__camera_window.__selected_pass != 0) //Swizzling stage (if swizzling is needed)
	{
		swizzler.draw(commandBuffer);
	}

	//Get data from GPU
	data_getter.compute(commandBuffer, __editor_window.NDCWindow);
	DeviceDataGetter::Data test;
	//if(__editor_window.isWindowSelected)
		data_getter.get_current_data(test);
	...
	ImGui::Image((ImTextureID)(resultDescriptorSet_ImGuiTexture), ImGui::GetContentRegionAvail());
	__editor_window.on_draw_time(commandBuffer, currentFrame);
	ImGui::End();
	
	//Draw Camera options window (if enabled)
	if (__editor_window.isCameraWindowVisible)
	{
		__camera_window.draw_ui();
	}
