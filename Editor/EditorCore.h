...
namespace SmoothieEditor
{
    ...
    class Core: public Smoothie::Editor_Core
    {
    protected:
        ...
        std::map<ToolsSearchKey, std::shared_ptr<Tool_Base>> m_tools =
        {
            {{"Tool_ScreenPicker", 1}, std::make_shared<Tool_ScreenPicker>()},
            {{"Tool_Camera", 0}, std::make_shared<Tool_Camera>()},
            {{"Tool_SceneGraph", 0}, std::make_shared<Tool_SceneGraph>()}
        };
    ...
    public:
        ...
        void on_scene_load(const std::string &settings_file) override;
        void on_scene_save(const std::string &settings_file) override;
        ...
        int create() override;
        void on_command_record_time(VkCommandBuffer commandBuffer, unsigned imageIndex) override;
        void on_command_submit_time() override;
