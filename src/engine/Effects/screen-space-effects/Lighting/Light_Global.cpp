...
int Smoothie::DeferredRendering::Light_Global::create_brdf()
{
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC(R"(Failed to cast drawing class to "DeferredRendering::Drawing" class.)");
        return 1;
    }
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create brdg pipeline layout.");
        return 1;
    }
    ...
    if (vkCreateGraphicsPipelines(SmoothieCore::getDevice(), nullptr, 1, &_brdf_pipeline_create_info, nullptr, &_brdf_pipeline) != VK_SUCCESS)
    {
        ...
        SMOOTHIE_ERROR_WITH_SRC("Failed to create BRDF pipeline.");
        return 1;
    }
...
int DeferredRendering::Light_Global::create_irradiance()
{
	...
	{
		SMOOTHIE_ERROR_WITH_SRC("Failed to create irradiance map image.");
		return 1;
	}
	...
	{
		SMOOTHIE_ERROR_WITH_SRC("Failed to create image map image view.");
		return 1;
	}
...
int DeferredRendering::Light_Global::create_prefiler()
{
	...
	{
		SMOOTHIE_ERROR_WITH_SRC("Failed to create prefilter image.");
		return 1;
	}
...
int DeferredRendering::Light_Global::create_descriptors()
{
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Descriptor set is not supported.");
        return 1;
    }
    ...
    for (const auto& [_path, _descriptor] : _layoutBindings)
    {
        ...
        {
            SmoothieCore::logError_Formated(R"({} "Failed to find image view "{}" in current map! Attempting to use default values.)", SMOOTHIE_LOCATION, _path);
...
int DeferredRendering::Light_Global::create_pipeline()
{
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create global illumination pipeline");
        return 1;
    }
...
int Smoothie::DeferredRendering::Light_Global::create(const std::string &shader)
{
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create shader file.");
        destroy();
        return 1;
    }
    ...
    if (create_brdf() != 0)
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create brdf data.");
        destroy();
        return 1;
    }
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create irradiance data.");
        destroy();
        return 1;
    }
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create prefiler data.");
        destroy();
        return 1;
    }
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create descriptors.");
        destroy();
        return 1;
    }
