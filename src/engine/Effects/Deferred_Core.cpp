...
int DeferredRendering::Drawing::create_samplers()
{
	...
	_create_info_Texture2DSampler.maxLod = 10;
	...
	{
	    SMOOTHIE_ERROR_WITH_SRC("Failed to create m_Texture2DSampler.");
	    return 1;
	}
	...
	_create_info_ClampToEdgeLINEAR.maxLod = 10;
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create m_ClampToEdgeLINEAR.");
        return 1;
    }
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create m_ClampToEdgeNEAREST.");
        return 1;
    }

	VkSamplerCreateInfo _create_info_RepeatNEAREST = _create_info_Texture2DSampler;
	_create_info_RepeatNEAREST.maxLod = 0.0f;
    if (vkCreateSampler(SmoothieCore::getDevice(), &_create_info_RepeatNEAREST, nullptr, &m_RepeatNEAREST) != VK_SUCCESS)
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create m_ClampToEdgeNEAREST.");
        return 1;
    }
...
int DeferredRendering::Drawing::create_camera()
{
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create CameraBuffer.");
        return 1;
    }

    if (m_CameraBuffer.map() != 0)
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to map Camera Buffer.");
        return 1;
    }
...
int DeferredRendering::Drawing::create_present_pipeline()
{
    ...
    _rendering_info.pColorAttachmentFormats = &m_PresentPipeline_RenderingFormat;
    const VkFormat _swap_chain_format = SmoothieCore::getSwapchainFormat();
    if (m_PresentPipeline_RenderingFormat == VK_FORMAT_UNDEFINED)
    {
        _rendering_info.pColorAttachmentFormats = &_swap_chain_format;
    }
    ...
    pipeline_createInfo.pNext = &_rendering_info;
    ...
    pipeline_createInfo.renderPass = nullptr;
	...
	{
	    SMOOTHIE_ERROR_WITH_SRC("Failed to create present pipeline.");
		return 1;
	}
...
void DeferredRendering::Drawing::begin_present_pass(VkCommandBuffer command_buffer, unsigned int imageIndex) const
{
    const auto _image =  SmoothieCore::getSwapchainImage(imageIndex);
    const auto _image_view= SmoothieCore::getSwapchainImageView(imageIndex);

    assert(_image != nullptr);
    assert(_image_view != nullptr);
    ...
    _attachment.imageView = _image_view;
...
void DeferredRendering::Drawing::present_pass(VkCommandBuffer command_buffer) const
{
    vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_PresentPipeline);
    SmoothieCore::setViewport(command_buffer);
    SmoothieCore::setScissor(command_buffer);
    ...
    assert(_descriptors[0] != nullptr);
    assert(_descriptors[1] != nullptr);

    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_PresentPipeline_Layout, 0, 2, _descriptors, 0, 0);
    vkCmdDraw(command_buffer, 6, 1, 0, 0);
}

void DeferredRendering::Drawing::end_present_pass(VkCommandBuffer command_buffer, unsigned int imageIndex) const
{
    ...
    _barrier_color.dstStageMask = VK_PIPELINE_STAGE_2_TRANSFER_BIT;
    _barrier_color.dstAccessMask = VK_ACCESS_2_TRANSFER_READ_BIT;

    _barrier_color.oldLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
    _barrier_color.newLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
    _barrier_color.srcQueueFamilyIndex = 0;
    _barrier_color.dstQueueFamilyIndex = 0;
    _barrier_color.image = _image;
    _barrier_color.subresourceRange = _color_subresource;
...
int DeferredRendering::Drawing::create_present_descriptor_set()
{
   ...
   {
        SMOOTHIE_ERROR_WITH_SRC("Failed to get global descriptor set data.");
       return 1;
   }
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Descriptor set is unsupported.");
        return 1;
    }
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create present descriptor set.");
        return 1;
    }
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create present descriptor pool!");
        return 1;
    }
...
int DeferredRendering::Drawing::create_lighting()
{
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("m_Light_Global is a nullptr.");
        return 1;
    }

    if (m_Light_Global->create(m_Lighting_Global_Shader) != 0)
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create global lighting data.");
        return 1;
    }
    return 0;
}
...
int DeferredRendering::Drawing::create_global_descriptor_sets()
{
   ...
   {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create global descriptor set.");
       return 1;
   }
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create global descriptor pool.");
        return 1;
    }
    ...
    for (const auto& [_path, _data] :m_Global_DescriptorWriteData)
    {
        ...
        switch (_data.type)
        {
            case VK_DESCRIPTOR_TYPE_SAMPLER:
            {
                ...
                {
                    SmoothieCore::logError_Formated("Sampler not found: {} ", _path);
                    continue;
                }
            ...
            case VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE:
            {
                ...
                {
                    SmoothieCore::logError_Formated("Sampled image not found: {}", _path);
                    continue;
                }
            ...
            case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER:
            {
                ...
                if (_buffers_map.find(_path) == _buffers_map.end())
                {
                    ...
                    SmoothieCore::logWarning_Formated("Updating buffer with a default uniform buffer: {}", _path);
                    continue;
                }
            ...
            default:
            {
                SmoothieCore::logError_Formated("Unexpected descriptor type for global descriptor set: {}", _path);
                continue;
            } break;
        }
...
int DeferredRendering::Drawing::create_command_buffers()
{
	...
	m_DrawingBuffers.resize(SmoothieCore::getFramesInFlightCount());
	for (auto& _data: m_DrawingBuffers)
	{
		...
		{
		    SMOOTHIE_ERROR_WITH_SRC("Failed to create drawing command pool.");
			return 1;
		}
		...
		{
			SMOOTHIE_ERROR_WITH_SRC("Failed to allocate command buffers.");
			return 1;
		}
...
void DeferredRendering::Drawing::populateInheritanceRenderingInfo_gBuffer(VkCommandBufferInheritanceRenderingInfo &inheritance_info) const
{
...
void DeferredRendering::Drawing::populateInheritanceRenderingInfo_HDR(VkCommandBufferInheritanceRenderingInfo &inheritance_info) const
{
    inheritance_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_INHERITANCE_RENDERING_INFO;
    inheritance_info.pNext = nullptr;
    inheritance_info.flags = 0;
...
void DeferredRendering::Drawing::compute_post_processing(VkCommandBuffer command_buffer) const
{
...
int DeferredRendering::Drawing::create_model_pipelines()
{
    ...
    {
        SMOOTHIE_ERROR_WITH_SRC("Failed to create sky pipeline.");
        return 1;
    }
...
int DeferredRendering::Drawing::create()
{
	...
	{
		SMOOTHIE_CRITICAL_WITH_SRC("Failed to create draw command buffer data.");
	    destroy();
		return 1;
	}
    SmoothieCore::logVerbose_Formated("Created draw command buffers.");

    if (create_gBuffer() != 0)
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to create gBuffer data.");
        destroy();
        return 1;
    }
    SmoothieCore::logVerbose_Formated("Created gBuffer images.");

    if (create_hdr() != 0)
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to create HDR pass.");
        destroy();
        return 1;
    }
    SmoothieCore::logVerbose_Formated("Created HDR image.");

    if (create_samplers() != 0)
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to create samplers.");
        destroy();
        return 1;
    }
    SmoothieCore::logVerbose_Formated("Created samplers.");

    if (create_camera() != 0)
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to create camera object.");
        destroy();
        return 1;
    }
    SmoothieCore::logVerbose_Formated("Created camera buffers.");

    //TODO: Fix this garbage
    if (m_PresentPipeline_ShaderFile.create(m_PresentPipeline_Shader) != 0)
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to create present pipeline shader.");
        destroy();
        return 1;
    }
    SmoothieCore::logVerbose_Formated("Created presentation pipeline.");
    ...
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to get WriteData_Descriptor for a global descriptor set.");
        destroy();
        return 1;
    }
    ...
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to create global descriptor sets.");
        destroy();
        return 1;
    }
    populate_global_descriptor_set();
    SmoothieCore::logVerbose_Formated("Created global descriptor set.");

    if (create_present_descriptor_set() != 0)
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to create present descriptor set.");
        destroy();
        return 1;
    }
    SmoothieCore::logVerbose_Formated("Created present pipeline descriptor set.");

    if (create_present_pipeline() != 0)
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to create present pipeline.");
        destroy();
        return 1;
    }
    SmoothieCore::logVerbose_Formated("Created present pipeline.");

    if (create_lighting() != 0)
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to create lighting.");
        destroy();
        return 1;
    }
    SmoothieCore::logVerbose_Formated("Created lighting pass.");

	if (create_model_pipelines() != 0)
	{
	    SMOOTHIE_CRITICAL_WITH_SRC("Failed to create model pipelines.");
	    destroy();
	    return 1;
	}
    SmoothieCore::logVerbose_Formated("Created model pipelines.");

    //Helpers
    if (m_Helper_HDR_CubeMaps.create(m_Helper_HDR_CubeMaps_Shader) != 0)
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to create CubeMaps helper.");
        destroy();
        return 1;
    }

    if (m_Helper_HDR_CubeMaps.create_cubmap_from_hdr("./scenes/glencairn_expressway_4k.hdr") != 0)
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to create cubemap from HDR.");
        destroy();
        return 1;
    }

    //TODO: Fix this garbage
    if (m_Helper_HDR_CubeMaps.copy_env_map_to_skybox_cubemap(m_Pipeline_Sky->get_Image_Sky(), {512, 512, 1}, VK_FORMAT_R32G32B32A32_SFLOAT) != 0)
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to copy env map to skybox.");
        destroy();
        return 1;
    }

    if (m_Helper_HDR_CubeMaps.calculate_irradiance_prefilter_maps() != 0)
    {
        SMOOTHIE_CRITICAL_WITH_SRC("Failed to calculate irradiance prefilter maps.");
        destroy();
        return 1;
    }
...
void DeferredRendering::Drawing::draw(VkCommandBuffer commandBuffer, unsigned int imageIndex) const
{
    ...
    compute_post_processing(commandBuffer);

    begin_present_pass(commandBuffer, imageIndex);
    present_pass(commandBuffer);
    end_present_pass(commandBuffer, imageIndex);
}
...
int DeferredRendering::Drawing::resize_callback()
{
	...
	if (create_gBuffer() != 0)
	{
	    SMOOTHIE_CRITICAL_WITH_SRC("Failed to recreate gbuffer images");
		return 1;
	}
	...
	if (create_hdr() != 0)
	{
	    SMOOTHIE_CRITICAL_WITH_SRC("Failed to recreate HDR images");
		return 1;
	}
