...
)
{
	...
	if (data == nullptr)
	{
	    SmoothieCore::logError_Formated("Failed to load image: {}", path);
		return 1;
	}
	std::lock_guard<std::mutex> lock(loadMutex);

	auto mipLevels = static_cast<uint32_t>(std::floor(std::log2(std::max(width, height)))) + 1;
	if (mipLevels > SmoothieCore::getFramesInFlightCount()) mipLevels = SmoothieCore::getFramesInFlightCount();
	...
	{
	    SmoothieCore::logError_Formated("{} Failed to copy image data to staging buffer.", SMOOTHIE_LOCATION);
		return 1;
	}
...
int Texture2D::create()
{
	...
	{
		SmoothieCore::logError_Formated( "Failed to load texture: {}", filepath);
		return 1;
	}

    SmoothieCore::logVerbose_Formated("Created Texture2D: {}", filepath);
	return 0;
}
...
void Texture2D::destroy()
{
    ...
    SmoothieCore::logVerbose_Formated("Destroyed Texture2D: {}", filepath);
}
